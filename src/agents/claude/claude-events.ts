...
export function redactAndSanitizeText(
  ...
  const patternRedacted = sanitized
    ...
    .replace(
      /\bsk-(?:live|test|proj|ant)?-[A-Za-z0-9_-]{8,}/gi,
      '[REDACTED]',
    )
    .replace(
...
function extractTextFromClaudeMessage(value: unknown): string | undefined {
...
function extractResultOutput(
  ...
  if ('result' in value) {
    ...
    // Claude stream-json often uses type:"result" with result as string summary
    // and structured_output separately; if result is object, treat as payload.
...
export interface ParseClaudeEventOptions {
...
/**
 * Parse one finished Claude stream-json / JSONL line into a normalized AgentEvent.
 ...
 *
 * Claude stream-json shapes (2.1.206) include:
...
export function parseClaudeEventLine(
  line: string,
  attemptId: AttemptId,
  options: ParseClaudeEventOptions = {},
): AgentEvent | null {
  ...
  try {
    ...
    return parseError(attemptId, trimmed, 'invalid stream-json line');
  }

  if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) {
    return parseError(attemptId, trimmed, 'stream-json value is not an object');
  }
  ...
  if (
    ...
    // Prefer structured_output when present (Claude JSON schema path).
    const structured =
    ...
    // type:result with only string result → output text (not schema result).
    if (typeof value.result === 'string') {
      const conversationId = extractConversationId(value);
      // Still try if result string is JSON object matching schema.
      try {
      ...
      return Object.freeze({
        ...
        attemptId,
        text: redactAndSanitizeText(value.result, 64 * 1024),
        ...(conversationId === undefined ? {} : {}),
      });
    }
    return parseError(attemptId, trimmed, 'result event missing structured output payload');
  }
  ...
  if (
    ...
    const text = extractTextFromClaudeMessage(value);
    if (text !== undefined) {
    ...
    if (
      ...
      if (typeof delta.text === 'string' && delta.text.length > 0) {
        return asOutput(attemptId, delta.text);
      }
      if (typeof delta.partial_json === 'string' && delta.partial_json.length > 0) {
        return asOutput(attemptId, delta.partial_json);
      }
    }
...
/**
 * Parse a multi-line stream-json / JSONL blob (recorded fixtures) into AgentEvents.
...
export function parseClaudeJsonl(
  ...
  for (const line of lines) {
    ...
    const event = parseClaudeEventLine(line, attemptId, options);
